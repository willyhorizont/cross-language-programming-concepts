...
RD=$(realpath "$SD/..")
V="2.7.93" # ! DON'T FORGET TO CHANGE VERSION BEFORE RUNNING !!!!
T=$(date "+%d %b %Y @ %I:%M %p")
cd "$RD" || exit
\. "$HOME/.nvm/nvm.sh"
...
M="
cancel learn beef, the language still work in progess and under active development;
small update in c-sharp;
small fix in objective-c runner.sh;
"
M=$(sed -e '/./,$!d' <<< "$M")
M="$H
$M"

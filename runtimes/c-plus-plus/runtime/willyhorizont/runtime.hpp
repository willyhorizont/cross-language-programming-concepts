...
struct XlClosureVarArgs {
    std::vector<CrossType> args;
    size_t index = 0;

    CrossType getNextArguments(); 
};

using XlClosure = std::function<CrossType(std::shared_ptr<XlClosureVarArgs>)>;
...
std::shared_ptr<XlClosureVarArgs> to_xl_closure_varargs(Args&&... args) {
    auto new_varargs = std::make_shared<XlClosureVarArgs>();
    
    (new_varargs->args.push_back(std::forward<Args>(args)), ...);
    
    return new_varargs;
}

template <typename... Args>
CrossType to_xl_list(Args&&... args) {
    XlList new_xl_list;

    new_xl_list.reserve(sizeof...(args));

    (new_xl_list.push_back(CrossType(std::forward<Args>(args))), ...);

    return CrossType(std::move(new_xl_list));
}

template <typename... Args>
CrossType to_xl_dict(Args&&... args) {
    XlDict new_xl_dict;

    (new_xl_dict.insert(std::forward<Args>(args)), ...);

    return CrossType(std::move(new_xl_dict));
}

template <typename... Args>
CrossType to_xl_dict_indexed(Args&&... args) {
...
std::string string_repeat(std::string s, size_t n) {
    std::string result = "";
...
std::string json_stringify(const CrossType& anything, JsonStringifyOptionalArgument option = {}) {
    ...
    while (!token_stack.empty()) {
        ...
        if (std::holds_alternative<std::shared_ptr<XlList>>(current_value)) {
            auto xl_list_ref = std::get<std::shared_ptr<XlList>>(current_value);
        ...
        if (std::holds_alternative<std::shared_ptr<XlDictIndexed>>(current_value)) {
            auto xl_dict_indexed_ref = std::get<std::shared_ptr<XlDictIndexed>>(current_value);
        ...
        if (std::holds_alternative<std::shared_ptr<XlDict>>(current_value)) {
            auto xl_dict_indexed_ref = std::get<std::shared_ptr<XlDict>>(current_value);
            if (xl_dict_indexed_ref->empty()) {
                result += "{}";
                continue;
            }
            ...
            std::vector<std::pair<std::string, CrossType>> xl_dict_items(xl_dict_indexed_ref->begin(), xl_dict_indexed_ref->end());

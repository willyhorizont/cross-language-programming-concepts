...
if [ "$CLEAR" = true ]; then
    for p in "${matched_profiles[@]}"; do
        ...
        # Trik Baru: Paksa buat folder konfigurasi profil kosong agar tidak error 'not found'
        # VS Code menyimpan konfigurasi profil di ~/.config/Code/User/profiles/
        if [ "$p" != "Default" ]; then
            mkdir -p "$HOME/.config/Code/User/profiles"
        fi

        # Ambil daftar ekstensi yang ada di profil tersebut dan hapus semuanya
        # Jika profil baru/kosong, perintah ini akan langsung dilewati dengan aman tanpa pesan error

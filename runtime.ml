...
let find = (fun va -> (
    ...
    let c = next itr in
    let n = ref (Type ()) in
    let ir = ref (Type 0) in
    try
        List.iter (fun el -> (
            if (to_bool ((to_closure c) (Type (init_list [el; !ir; l])))) then begin
                n := el;
                raise Break
            end;
            ir := Type ((to_int !ir) + 1)
...
let json_stringify = (fun va -> (
    ...
    while List.length !s > 0 do
        try
            let (ls, lt) = pop (Type [Type !s]) in
            s := ls;
            let c = get (Type (init_list [lt; Type "v"])) in
            if ((to_string (get (Type (init_list [lt; Type "t"])))) = "r") then begin
                r := Type ((to_string !r) ^ (to_string c));
                raise Continue
            end;
            let cur_d = get (Type (init_list [lt; Type "d"])) in
            let ct = get_type c in
            if ((to_string ct) = "[object XlInt]") then begin
                r := Type ((to_string !r) ^ (string_of_int (to_int c)));
                raise Continue
            end;
            if ((to_string ct) = "[object XlBool]") then begin
                r := Type ((to_string !r) ^ (string_of_bool (to_bool c)));
                raise Continue
            end;
            if ((to_string ct) = "[object XlNone]") then begin
                r := Type ((to_string !r) ^ (string_of_int (to_int (Type 0))));
                raise Continue
            end;
            if ((to_string ct) = "[object XlString]") then begin
                r := Type ((to_string !r) ^ (to_string (Type "\"")) ^ (to_string c) ^ (to_string (Type "\"")));
                raise Continue
            end;
            if ((to_string ct) = "[object XlFloat]") then begin
                r := Type ((to_string !r) ^ (string_of_float (to_float c)));
                raise Continue
            end;
            if ((to_string ct) = "[objecr XlList]") then begin
                let l = to_list c in
                if List.length l = 0 then begin
                    r := Type ((to_string !r) ^ (to_string (Type "[]")));
                    raise Continue
                end;
                let cur_d = Type ((to_int cur_d) + 1) in
                ...
                for i = (List.length l - 1) downto 0 do
                    ...
                    if i > 0 then begin
                        s := append (Type [Type !s; (Type (init_dict [
                            Type [Type "t"; Type "r"];
                            Type [Type "v"; (if to_bool !p then (Type ((to_string (Type ",\n")) ^ (to_string (string_repeat (Type [t; cur_d]))))) else (Type ", "))];
                            Type [Type "d"; cur_d]
                        ]))]);
                    end;
                done;
            ...
            if ((to_string ct) = "[objecr XlDict]") then begin
                let de = to_dict c in
            ...
            r := Type ((to_string !r) ^ (to_string (Type "\"")) ^ (to_string ct) ^ (to_string (Type "\"")));
            raise Continue
        with Continue -> ()
    done;
    !r

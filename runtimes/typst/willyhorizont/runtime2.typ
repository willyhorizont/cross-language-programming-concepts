...
#let setup-document = (document) => {
    ...
    if ((elements.len() > 0) and ((elements.at(0).func() == parbreak) or (elements.at(0) == [ ]) or (elements.at(0) == [#{"\n"}]))) {
    ...
    let mapped-elements = elements.map((item) => {
        ...
        if ((new-item.len() > 0) and ((new-item.at(0).func() == parbreak) or (new-item.at(0) == [ ]) or (new-item.at(0) == [#{"\n"}]))) {
...
#let setup-command-prompt = (document) => {
    let mapped-elements = setup-document(document)
    panic(mapped-elements)
    let new-document = mapped-elements.sum()
    panic(new-document)
